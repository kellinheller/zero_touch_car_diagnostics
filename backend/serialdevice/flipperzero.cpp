...
SerialDevice::SerialDevice(const Zero::DeviceInfo &info, QObject *parent):
    QObject(parent),
...
{
    connect(m_state, &DeviceState::deviceInfoChanged, this, &SerialDevice::onDeviceInfoChanged);
    connect(m_state, &DeviceState::deviceInfoChanged, this, &SerialDevice::deviceStateChanged);

    connect(m_rpc, &ProtobufSession::sessionStateChanged, this, &SerialDevice::onSessionStatusChanged);

    onDeviceInfoChanged();
}

DeviceState *SerialDevice::deviceState() const
{
    return m_state;
}

ProtobufSession *SerialDevice::rpc() const
{
    return m_rpc;
}

UtilityInterface *SerialDevice::utility() const
{
    return m_utility;
}

// TODO: Handle -rcxx suffixes correctly
bool SerialDevice::canUpdate(const Updates::VersionInfo &versionInfo) const
{
...
bool SerialDevice::canInstall(const Updates::VersionInfo &versionInfo) const
{
...
bool SerialDevice::canRepair(const Updates::VersionInfo &versionInfo) const
{
    Q_UNUSED(versionInfo)
    return m_state->isRecoveryMode();
}

void SerialDevice::fullUpdate(const Updates::VersionInfo &versionInfo)
{
    const auto &storageInfo = m_state->deviceInfo().storage;
...
void SerialDevice::onDeviceInfoChanged()
{
    if(m_state->isOnline()) {
...
void SerialDevice::onSessionStatusChanged()
{
    if(m_rpc->isError()) {
...
void SerialDevice::registerOperation(AbstractOperation *operation)
{
