...
#include "serialdevice/helper/deviceinfohelper.h"
...
SerialDevice *DeviceRegistry::currentDevice() const
{
...
{
    const auto it = std::find_if(m_devices.begin(), m_devices.end(), [&](Flipper::SerialDevice *dev) {
...
{
    auto it = std::remove_if(m_devices.begin(), m_devices.end(), [](Flipper::SerialDevice *arg) {
...
void DeviceRegistry::processDevice()
{
    ...
    const auto it = std::find_if(m_devices.begin(), m_devices.end(), [&info](Flipper::SerialDevice *arg) {
    ...
    if(it != m_devices.end()) {
        ...
        auto *device = new SerialDevice(info, this);
